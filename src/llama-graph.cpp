...
ggml_tensor * llm_graph_context::build_rs(
    ...
    if (rs_zero >= 0 && rs_size > 0) {
        GGML_ASSERT(states->ne[1] % rs_size == 0); // plane-interleaved invariant
        const uint32_t rows_per_cell = states->ne[1] / rs_size; // = 1 + n_rs_seq
        LLAMA_LOG_DEBUG("%s: zeroing %u rows (primary + %u snapshots) for rs_zero cell %d, rs_size=%u\n",
                        __func__, rows_per_cell, rows_per_cell - 1, rs_zero, rs_size);
